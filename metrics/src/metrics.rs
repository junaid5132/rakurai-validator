...
impl InfluxDbMetricsWriter {
    fn new() -> Self {
        Self {
            ...
            #[cfg(not(feature = "without_influxdb"))]
            rakurai_clickhouse: Self::build_rakurai_clickhouse(),
        }
    }
    ...
    /// Builds the Rakurai ClickHouse client + its own tokio runtime (the metrics
    /// agent thread is sync, but the official `clickhouse` crate is async).
    #[cfg(not(feature = "without_influxdb"))]
    fn build_rakurai_clickhouse() -> Option<RakuraiClickHouseWriter> {
        let config = get_rakurai_metrics_config()
            .map_err(|err| info!("rakurai metrics disabled: {err}"))
            .ok()?;
        ...
        let runtime = tokio::runtime::Builder::new_current_thread()
            .enable_all()
            .build()
            .map_err(|err| warn!("rakurai ClickHouse runtime init failed: {err}"))
            .ok()?;
        let client =
            clickhouse::build_client(&config.host, &config.db, &config.username, &config.password);
...
/// Splits `points` into Solana Influx lines, keyed by whether they're an
/// `extra_stats` measurement. Rakurai (`rakurai*`) points are excluded here —
/// they go to ClickHouse via [`InfluxDbMetricsWriter::write_rakurai_points`].
...
impl InfluxDbMetricsWriter {
    /// Routes any `rakurai*` points to ClickHouse. The Influx line-protocol
    /// serializer above already skips them, so this is the only place they're sent.
    #[cfg_attr(feature = "without_influxdb", allow(unused_variables))]
    fn write_rakurai_points(&self, points: &[DataPoint], host_id: &str) {
