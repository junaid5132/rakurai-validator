...
impl VoteWorker {
    ...
    fn process_packets(
        ...
        *rebuffered_packet_count += deferred_restore_count;
        ...
        while let Some((vote_pubkey, packet, source, arrival_nanos, source_ipv4)) = all_vote_packets.pop_front() {
            ...
            *rebuffered_packet_count += retryable_vote_count;
            ...
            if has_reached_end_of_slot(reached_max_poh_height, bank) {
                self.storage.reinsert_packets(all_vote_packets.into_iter().map(
                    |(_pubkey, vote, source, arrival_nanos, source_ipv4)| {
                        (vote, source, arrival_nanos, source_ipv4)
                    },
                ));
                return true;
            }
...
mod tests {
    use {
        ...
        solana_poh::{
            poh_recorder::{create_test_recorder, LeaderState, SharedLeaderState},
    ...
    fn test_retained_vote_after_direct_bank_replacement() {
        ...
        let (_exit, poh_recorder, _controller, _recorder, _service, _entry_receiver) =
            create_test_recorder(bank_a.clone(), Arc::new(blockstore), None, None);
        let mut worker = VoteWorker::new(
            ...
            VotePacketReceiver::new(never(), Arc::default(), false),
            VotePacketReceiver::new(never(), Arc::default(), false),
            VoteStorage::new(&bank_a),
            bank_forks,
            Consumer::new(
                Committer::new(None, bounded(1).0, None, None),
