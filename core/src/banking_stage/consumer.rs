    ...
    pub fn process_and_record_aged_transactions_with_policy(
        ...
        if let Some(results) = admission_results {
            if self.capture_gui_timestamps {
                let mb_start_timestamp = wallclock_timestamp_nanos();
                microblock_start_timestamps_nanos.resize(txs.len(), mb_start_timestamp);
            }
            let (mut output, cu_err_indexes) = 
