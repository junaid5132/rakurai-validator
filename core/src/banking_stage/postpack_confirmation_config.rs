use {
    super::PostPackConfirmationConfig, solana_pubkey::Pubkey, solana_runtime::bank::Bank,
};
...
mod ffi {
    use {
        super::{CachedUuidMevShareGroup, PostPackConfirmationConfig},
    ...
    unsafe extern "C" {
        ...
        #[allow(improper_ctypes)]
        #[allow(improper_ctypes_definitions)]
        pub fn load_postpack_confirmation_config(
            bank: &Bank,
            vote_account: &Pubkey,
        ) -> Option<PostPackConfirmationConfig>;
    }
}
...
/// Loads post-pack confirmation endpoint entries from the on-chain client-config PDA.
pub fn load_postpack_confirmation_config(
    bank: &Bank,
    vote_account: &Pubkey,
) -> Option<PostPackConfirmationConfig> {
    ...
    {
        // SAFETY: exported by rakurai_scheduler entrypoint from the same revision.
        return unsafe { ffi::load_postpack_confirmation_config(bank, vote_account) };
    }
    #[cfg(not(feature = "build_validator"))]
    {
        let _ = (bank, vote_account);
