...
**The tool a validator uses to join Rakurai and to set what share of the block reward they keep.**

Its two main jobs: creating the **Rakurai Activation Account (RAA)** that authorizes your node to run the scheduler at all, and updating `block_reward_commission_bps`, which only the validator can change and can be changed at any time.
...
> [!WARNING]
> **Check all three before every command**
>
> An incorrect keypair, RPC URL, or program ID results in a failed transaction. The most common mistake is using the **mainnet program ID against the testnet RPC** (or the reverse), because each cluster has its own Rakurai Activation Program.
...
> [!WARNING]
> **Creating an RAA is mandatory**
>
> A Rakurai-Solana validator cannot run without an RAA, and you need a separate one for **each validator identity and each cluster**. If you already created one, recover its pubkey with `rakurai-activation -p <PROGRAM_ID> show -i <IDENTITY_PUBKEY> -um` rather than initializing a second time.
...
Validators can update their share of the block reward at any time, independent of Rakurai. Only the validator can change their commission. The updated commission applies from the **current epoch** if no [RCA](../programs/reward_distribution/README.md#3-rca--block-rewards-for-stakers) has been opened yet, or from the **next epoch** if one already exists.
