...
**This is the switchboard for which services can reach a Rakurai validator.** It manages on-chain **block-engine** (recv bundles), **post-pack / P2C** (send txns for backrun), and **virtual-priority** (percent of a tip account’s tip) settings.

The operating rule that matters most: every write **replaces the whole config**, so you always submit **current + new**. Submitting only your new entry silently removes everything else, including endpoints other partners are live on.
...
`global update`, `validator update`, `proposal submit`, and `proposal` update **replace the entire on-chain `Config`**. There is no patch or merge on write. Program-level semantics: [Client Config — full replace](../programs/rakurai_client_config/README.md#3-full-replace--current--new).
...
3. Append the new named set, keeping every set you still want
4. Submit that file with `--config-file`

> [!CAUTION]
> **A partial file silently deletes the rest**
>
> Never submit a JSON that contains only the new set. That file becomes the **whole** PDA contents and every other set on the account is deleted. Always dump the current config first and keep a copy — `--dry-run` is not available on these writes.
...
| P2C | `url`, optional `enable_tpu_p2c_update` (per URL). Scheduler **sends** packed txs here for arbitrage / backrun (point of no return; no front-running). When `enable_tpu_p2c_update` is true, the validator also opens `StartExpiringTpuPacketStream` if your server implements it. See [Post-pack confirmations](../../transaction_inclusion/post_pack/README.md) · [Using P2C](../../transaction_inclusion/post_pack/using_p2c.md). |
...
### 5.2. Validator live PDA (manager)
...
### 6.1. Manager — create the live validator PDA and name the operator
...
### 6.2. Operator — dump what is live today
...
### 6.3. Operator — add the new set, keeping current
...
After **approve**, `validator show --vote <VOTE>` should match the JSON from step 6.3. `union --vote <VOTE>` prints that same validator payload.
